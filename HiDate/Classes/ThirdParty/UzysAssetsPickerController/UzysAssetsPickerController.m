...
- (void)setupLayout
{
    ...
    {
        if (_maximumNumberOfSelectionPhoto == 0) {
            [self.btnCamera setImage:[UIImage Uzys_imageNamed:appearanceConfig.videoImageName] forState:UIControlStateNormal];
        } else if (_maximumNumberOfSelectionVideo == 0) {
    ...
    self.btnCamera.hidden = self.isDisalbeUseCamera;
}
...
- (void)setupOneMediaTypeSelection
{
    if(_maximumNumberOfSelectionMedia > 0)
    {
        ...
        if(_maximumNumberOfSelection > 1)
            self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"选择媒体", @"UzysAssetsPickerController", nil);
        else
            self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"选择一个媒体", @"UzysAssetsPickerController", nil);
    ...
    else
    {
        if(_maximumNumberOfSelectionPhoto == 0)
        {
            ...
            if(_maximumNumberOfSelection > 1)
                ...
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"选择一段视频", @"UzysAssetsPickerController", nil);
        }
        else if(_maximumNumberOfSelectionVideo == 0)
        {
            //            self.assetsFilter = [ALAssetsFilter allPhotos];
            ...
            self.labelSelectedMedia.hidden = NO;
            if(_maximumNumberOfSelection >1)
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"选择照片", @"UzysAssetsPickerController", nil);
            else
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"选择一张照片", @"UzysAssetsPickerController", nil);
        }
        else
        {
...
- (void)initNoAssetView
{
    ...
    title.text              = NSLocalizedStringFromTable(@"缺少照片或视频", @"UzysAssetsPickerController", nil);
    ...
    message.text            = NSLocalizedStringFromTable(@"您可以使用iTunes同步照片和视频到你的iPhone。", @"UzysAssetsPickerController",nil);
...
- (void)showNotAllowed
{
    ...
    title.text              = NSLocalizedStringFromTable(@"该程序无权访问您的照片或视频.", @"UzysAssetsPickerController",nil);
    title.font              = [UIFont boldSystemFontOfSize:17.0];
    title.textColor         = [UIColor colorWithRed:129.0/255.0 green:136.0/255.0 blue:148.0/255.0 alpha:1];
    title.textAlignment     = NSTextAlignmentCenter;
    title.numberOfLines     = 5;
    
    message.text            = NSLocalizedStringFromTable(@"您可以在隐私设置中访问.", @"UzysAssetsPickerController",nil);
...
- (void)showNoAssetsIfNeeded
{
    ...
    voidBlock setNoImage = ^{
        ...
        title.text = NSLocalizedStringFromTable(@"无照片", @"UzysAssetsPickerController",nil);
        UILabel *msg = (UILabel *)[weakSelf.noAssetView viewWithTag:kTagNoAssetViewMsgLabel];
        msg.text = NSLocalizedStringFromTable(@"您可以使用iTunes同步照片和视频到您的iPhone.",@"UzysAssetsPickerController", nil);
    };
    voidBlock setNoVideo = ^{
        ...
        DLog(@"no video");
        UILabel *title = (UILabel *)[weakSelf.noAssetView viewWithTag:kTagNoAssetViewTitleLabel];
        title.text = NSLocalizedStringFromTable(@"无视频", @"UzysAssetsPickerController",nil);
        UILabel *msg = (UILabel *)[weakSelf.noAssetView viewWithTag:kTagNoAssetViewMsgLabel];
        msg.text = NSLocalizedStringFromTable(@"您可以使用iTunes同步视频到您的iPhone.",@"UzysAssetsPickerController", nil);
    ...
    if(self.assets.count ==0)
    {
        ...
        else
        {
            if(self.maximumNumberOfSelectionMedia >0)
            {
                ...
                msg.text = NSLocalizedStringFromTable(@"您可以使用iTunes媒体同步到您的iPhone.",@"UzysAssetsPickerController", nil);
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BOOL shouldSelectAsset = YES;
    if (!didExceedMaximumNumberOfSelection) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(uzysAssetsPickerControllerShouldSelectAsset:)]) {
            ALAsset *selectedAsset = [self.assets objectAtIndex:indexPath.item];
            shouldSelectAsset = [self.delegate uzysAssetsPickerControllerShouldSelectAsset:selectedAsset];
        }
    }
    
    return !didExceedMaximumNumberOfSelection && shouldSelectAsset;
}
    ...
    switch (btn.tag) {
        case kTagButtonCamera:
        {
            if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
                ...
                NSString *message = NSLocalizedStringFromTable(@"设备无相机", @"UzysAssetsPickerController", nil);
